...
export class HamburgerMenuComponent implements AfterViewInit, OnInit, OnDestroy {
      ...
      children: [
        {
          name: 'Self Service',
          children: [
            { name: 'Summary', path: '/ServiceInteraction/SelfService' },
            { name: 'EDI', path: '/ServiceInteraction/EDI' }
          ]
        },
  ...
  private togglePanels(_value: boolean, path) {
    ...
    this.viewPanels.forEach(element => {
      ...
      } else if (path === '/ServiceInteraction/Calls') {
        if (element.id === 'cdk-accordion-child-2') {
          element.open();
        } else {
          element.close();
        }
      } else if (path === '/ServiceInteraction/EDI' || path === '/ServiceInteraction/SelfService') {
        if (element.id === 'cdk-accordion-child-2' || element.id === 'cdk-accordion-child-3') {
          element.open();
        } else {
          element.close();
        }
